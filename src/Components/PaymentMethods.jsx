...
const PaymentMethods = ({ onClose, productId }) => {
  return (
    <div className='h-screen w-screen fixed bottom-0 left-0 right-0 top-0 z-50'>
      ...
      <div role="dialog" className="absolute top-32 left-0 right-0 mx-auto w-5/6 md:w-1/2 xl:w-1/3 min-h-[30%] rounded-lg bg-white text-black p-4 md:p-8">
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <p className='text-center mb-6 text-sm md:text-lg text-gray-400 font-bold'>
            ...
            <div className='flex flex-col items-center justify-center'>
                {methods.map(method => 
                  <Link to={`/products/${productId}/payment/${method.id}`} key={method.id} className='w-full flex items-center justify-center gap-4 mb-4 p-2 bg-[#EAEFF2] rounded-lg hover:bg-main'>
